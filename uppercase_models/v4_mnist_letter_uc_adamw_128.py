...
v4_mnist_digit_adamw_128.py — see that file's own docstring /
...
Output: ./v4_mnist_letter_uc_adamw_128/  (created next to this script)
"""
...
OUTPUT_ROOT      = Path(__file__).resolve().parent / f"v4_mnist_letter_uc_adamw_{IMG_SIZE}"

LABEL_MAP = list("ABCDEFGHIJKLMNOPQRSTUVWXYZ")


def build_config(img_size: int, batch_size: int) -> dict:
    prefix = f"v4_mnist_letter_uc_adamw_{img_size}"
    return {
        "img_size":    img_size,
...
if __name__ == "__main__":
    ...
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    sys.stdout = _Tee(OUTPUT_ROOT / f"v4_mnist_letter_uc_adamw_{IMG_SIZE}_cli_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
