...
HardwareMonitor.sample() takes multiple readings across an epoch;
epoch_summary() reduces those samples to min/avg/max per metric (so both
"how low did it dip" and "what's the real average" are visible, not just
one number). get_hw_stats() (kept below, unchanged) is a single cheap
snapshot for any caller that doesn't need the multi-sample version.
...
NOTE ON VALIDATION: the nvidia-smi fields below (power.draw, clocks.sm/
mem, fan.speed, clocks_throttle_reasons.*, and the fields added since)
are documented, standard nvidia-smi --query-gpu fields, but — unlike
utilization.gpu/temperature.gpu, which this project has confirmed
working against real hardware (RTX 4080) — these specific fields have
NOT all been confirmed against a real run in this project. Parsing is
...
# Module-level, not per-HardwareMonitor-instance — continuous for the
# whole run, not reset every epoch (see HardwareMonitor's own docstring).
# Initialized once at import time, so the very first sample() call of the
# whole run already has a valid baseline to compute a delta against.
...
def _cuda_index() -> int:
    ...
    Every nvidia-smi --query-gpu call in this file must pass -i
    explicitly — omitting it queries EVERY GPU on the system and returns
    one CSV row per card, which silently mis-parses as a single row on
    any machine with 2+ GPUs. Uses torch.cuda.current_device() — set once
    via setup_device()'s gpu_id parameter (torch.cuda.set_device()) at
    the very start of a run — so every telemetry call in this file
    automatically targets whichever GPU this process actually selected,
    without needing the GPU index threaded through every function
    signature.
    """
...
def get_hw_stats() -> dict:
    ...
    VRAM peaks via torch.cuda, CPU/RAM via psutil. Kept as-is for any
    caller that only needs one cheap reading rather than
    HardwareMonitor's multi-sample min/avg/max.
    """
    stats = {
...
# expensive; multiplying that by the number of fields would not be).
_NVSMI_FIELDS = (
...
def _query_pcie_throughput() -> tuple:
    ...
    hardware — confirmed absent from this machine's own
    `nvidia-smi --help-query-gpu` field list, and DCGM's profiling
...
# "GPU Process Memory" performance-counter set — the same one Task
# Manager's GPU tab reads from, instanced per-PID
# ("pid_<PID>_luid_..._phys_N"). nvidia-smi cannot see the shared figure
# at all: "shared" is WDDM spilling GPU allocations into regular system
# RAM when dedicated VRAM is under pressure. Filtered to THIS process's
# own PID (os.getpid(), resolved fresh at import time) via the
# "pid_<PID>_*" instance-name prefix, so the number this function reports
# is this run's own footprint — something that reducing its own batch
# size can actually move.
#
# Measured at ~2.5s per call vs nvidia-smi's ~0.07s — so this CANNOT be
# called synchronously inside sample() the way every other field is; a
# blocking 2.5s call would itself get counted as "wait" time in each
# training script's own data-wait-vs-compute split (sample() runs in the
# gap between one batch's compute finishing and the next batch's
...
# Module-level, not per-HardwareMonitor-instance: a new HardwareMonitor
# is created every epoch (see this class's own docstring), but a
# background query launched near the end of one epoch may not finish
# before that epoch's instance goes out of scope. Keeping the in-flight
# handle at module level lets it survive across epoch boundaries and
# actually be collected, instead of being abandoned mid-query every time
# a new HardwareMonitor replaces the old one.
...
def _cleanup_gpu_mem_proc() -> None:
    """Explicit process-exit cleanup — terminates any still-running
    background GPU-memory query instead of leaving it for the OS to reap
    whenever it gets around to it. Registered below via atexit, so this
    runs when the training script's own process actually ends, not left
    to chance."""
...
class HardwareMonitor:
    ...
    MODULE level, not on the instance — continuous tracking for the
    whole run, not reset every epoch — so that per-epoch re-instantiation
    doesn't matter; the underlying state survives across it automatically.
    ...
    def sample(self) -> dict:
        ...
        # memory.used, confirmed system-wide by both NVIDIA's own docs
        # and this machine's own --help-query-gpu output, "Total memory
        # allocated by active contexts") minus this process's own
        # reserved VRAM (torch's max_memory_reserved(), genuinely
        # per-process) = what everything ELSE on the GPU is using.
        # Floored at 0: the two readings come from independent sources
        # taken microseconds apart, so small skew could otherwise show a
...
def _format_vram_trend(vram_history: list, window: int = 5) -> str:
    ...
    judges the shape of the curve themselves (an algorithmic label would
    need an invented threshold for what counts as "sudden," which has no
    principled basis). Returns "" if there isn't enough history yet to
...
def check_vram_safety(nvsmi_used_peak_gb: float, nvsmi_total_gb: float, epoch: int,
    ...
    """
    Returns "stop", "warn", or "ok" based on this epoch's PEAK nvidia-smi
    VRAM usage (nvsmi_used_peak_gb, from HardwareMonitor.epoch_summary()'s
    nvsmi_vram_used_gb_max — sampled at the same interval as every other
    telemetry field, not a single end-of-epoch snapshot, so a genuine
    mid-epoch spike that drops back down isn't missed) against this GPU's
    real physical capacity (nvsmi_total_gb). "ok" (no-op) if either value
    is unavailable (< 0). Prints a warning first for "warn"/"stop".

    Compares nvidia-smi's own memory.used/memory.total, not torch.cuda's
    own reserved-memory figure — the two measurement systems can
    disagree (torch.cuda's reserved figure can read higher than physical
    capacity on Windows/WDDM), and mixing them risks a false stop with
    real headroom still available.

    vram_history: optional list of every prior epoch's peak VRAM
    (history['nvsmi_vram_used_gb_max'], including this epoch as its last
    entry) — when given, a "warn" or "stop" print is followed by a trend
    line (see _format_vram_trend()) showing the last few epochs' raw
    readings and the recent per-epoch delta versus this epoch's own jump,
    so it's visible from the log whether this was a gradual creep or a
    sudden single-epoch spike. No-op (omitted) if not given.

    Two-tier by design: warn_reserve_gb (default 1.0 GB) is advisory —
    the caller reactively reduces batch size (see common/batch_sizing.py's
    reduce_batch_size()) and continues, rather than stopping.
    stop_reserve_gb (default 0.25 GB) is the actual hard-stop trigger,
    much closer to real exhaustion — the point is to stop the GPU from
    being consumed so completely that it bottlenecks the rest of the
    system, not to treat a modest headroom buffer as a crash boundary.

    shared_mem_warn_gb (default 6.0 GB): reads the module-level
    shared-GPU-memory reading directly (_last_gpu_mem[1], from
    _poll_gpu_memory_windows()/HardwareMonitor's async Windows polling).
    nvidia-smi cannot see shared memory at all, so this is a genuinely
    separate signal from everything else in this function, evaluated
    independently of whether nvsmi_used_peak_gb/nvsmi_total_gb are
    available. Only ever contributes to "warn", never "stop" —
    shared-memory spillover is WDDM's own mechanism for avoiding a hard
    CUDA OOM crash by degrading to slower memory instead of failing, so
    heavy shared usage is a real performance problem, not the same kind
    of crash risk dedicated-VRAM exhaustion is. Only acts on a high
    shared-memory reading when nvidia-smi ALSO confirms dedicated VRAM is
    genuinely under pressure (within warn_reserve_gb of capacity) — a
    high reading with dedicated VRAM comfortably clear just prints an
    informational line instead of triggering a reduction. Fails open if
    nvidia-smi's own reading is unavailable: the shared-memory reading is
    acted on alone, with no corroborating signal either way. -1 (no
    successful query yet, or non-Windows) is "no signal" — same
    -1/unknown convention as everywhere else in this module.

    warn_persist_epochs (default 3): the nvidia-smi/warn_reserve_gb
    branch only returns "warn" once the last warn_persist_epochs entries
    of vram_history (current epoch included) are ALL over threshold,
    instead of reacting to one epoch's reading — smooths out single-epoch
    noise (nvidia-smi's memory.used reflects PyTorch's cached-but-idle
    reserved pool, which nothing in the training loop releases via
    empty_cache() between epochs, so a single plateau reading could
    otherwise re-trigger "warn" on every subsequent epoch even as batch
    size is cut repeatedly, without ever bringing the reading back down).
    Deliberately NOT applied to the stop_reserve_gb branch — stop is the
    real OOM-prevention net and should fire immediately on any single
    epoch that crosses it, not wait for confirmation. Also not applied to
    the shared_mem_warn_gb branch — that check has no history array
    threaded through it to smooth against; it's instead gated on
    corroborating dedicated-VRAM pressure, as described above.
    """
...
def check_cpu_ram_safety(device: torch.device, swap_baseline_gb: float,
    ...
    dropped below the configured reserve. No-op (False) on a GPU run —
    this check is specifically about CPU-only training exhausting system
    RAM/swap; a GPU run's own swap fluctuation is an unrelated signal and
    must not stop a healthy CUDA run. Prints a warning first. No-op
    (False) if psutil isn't available.
    """
...
def setup_device(cpu_reserve_pct: int = 25, use_amp: bool = True, gpu_id: int = None) -> torch.device:
    ...
    reservation / DataLoader worker count — printed on every run (GPU or
    CPU), since NUM_WORKERS (every training script's DataLoader worker
    count) uses this exact same usable-core number regardless of whether
    model compute happens on GPU or CPU. reserve_cpu_threads()
    ...
    gpu_id: optional physical GPU index to train on, from each script's
    --gpu CLI flag. Calling torch.cuda.set_device(gpu_id) HERE, before
    anything else in this function runs, is what makes every other
    GPU-index-aware call in this codebase (_cuda_index() in this module,
    and the get_device_properties()/nvidia-smi calls that use it —
    including common/batch_sizing.py's probe) automatically target the
    right card without gpu_id being threaded through every single
    function signature: torch.cuda.current_device() reflects this call
    for the rest of the process's lifetime. None (the default) leaves
    whatever device CUDA already considers current untouched — correct
    for a single-GPU machine, or for a multi-GPU machine run under
    `torchrun`/DDP (see common/distributed.py), where the launcher itself
    already sets the correct per-process device before this function is
    called.
    """
