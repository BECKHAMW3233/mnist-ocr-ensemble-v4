...
v4_mnist_digit_muon_128.py (the digit ensemble), retargeted at a 26-class
lowercase a-z letter-identity problem.

Architecture — OCRConvNetMuon (same as the digit ensemble except the
...
implementation). Identical hyperparameters to v4_mnist_digit_muon_128.py:
...
No digit classes, no uppercase classes, no supplementary sources, and no
...
Output: ./v4_mnist_letter_lc_muon_128/  (created next to this script)
"""
...
OUTPUT_ROOT   = Path(__file__).resolve().parent / f"v4_mnist_letter_lc_muon_{IMG_SIZE}"


def build_config(img_size: int, batch_size: int) -> dict:
    prefix = f"v4_mnist_letter_lc_muon_{img_size}"
    return {
        "img_size":    img_size,
...
def load_letters(img_size: int):
    """
    Loads the lowercase 26-class letter split from EMNIST ByClass — this
...
if __name__ == "__main__":
    ...
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    sys.stdout = _Tee(OUTPUT_ROOT / f"v4_mnist_letter_lc_muon_{IMG_SIZE}_cli_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
