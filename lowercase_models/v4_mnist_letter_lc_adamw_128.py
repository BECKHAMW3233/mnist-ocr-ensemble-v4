...
blocks. Same architecture and hyperparameters as v4_mnist_digit_adamw_128.py
(the digit ensemble), retargeted at a 26-class lowercase a-z letter-
identity problem.
...
already returns — the same pattern v4_mnist_digit_soap_128.py /
v4_mnist_digit_muon_128.py already use for their own no-supplementary-data
...
Output: ./v4_mnist_letter_lc_adamw_128/  (created next to this script)
"""
...
OUTPUT_ROOT      = Path(__file__).resolve().parent / f"v4_mnist_letter_lc_adamw_{IMG_SIZE}"

LABEL_MAP = list("abcdefghijklmnopqrstuvwxyz")


def build_config(img_size: int, batch_size: int) -> dict:
    prefix = f"v4_mnist_letter_lc_adamw_{img_size}"
    return {
        "img_size":    img_size,
...
if __name__ == "__main__":
    ...
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    sys.stdout = _Tee(OUTPUT_ROOT / f"v4_mnist_letter_lc_adamw_{IMG_SIZE}_cli_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
