...
v4_mnist_digit_soap_128.py (the digit ensemble), retargeted at a 26-class
lowercase a-z letter-identity problem.

Architecture — OCRConvNetTriplePyramid (same as the digit ensemble except
...
Identical hyperparameters to v4_mnist_digit_soap_128.py:
...
Output: ./v4_mnist_letter_lc_soap_128/  (created next to this script)
"""
...
# path; this local copy must be kept in sync with it manually).
...
OUTPUT_ROOT   = Path(__file__).resolve().parent / f"v4_mnist_letter_lc_soap_{IMG_SIZE}"

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")


def build_config(img_size: int, batch_size: int) -> dict:
    prefix = f"v4_mnist_letter_lc_soap_{img_size}"
    return {
        "img_size":    img_size,
...
if __name__ == "__main__":
    ...
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    sys.stdout = _Tee(OUTPUT_ROOT / f"v4_mnist_letter_lc_soap_{IMG_SIZE}_cli_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
